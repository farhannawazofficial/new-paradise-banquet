'use client';

// using regular img tag for logo to avoid missing-image crashes during development
import { useState } from 'react';

export default function Navbar() {
  const [isOpen, setIsOpen] = useState(false);

  // Map nav items to section IDs on the page
  const navLinks = [
    { id: 'hero', label: 'HOME' },
    { id: 'about', label: 'ABOUT' },
    // Services removed per request
    { id: 'services', label: 'PACKAGES' },
    { id: 'gallery', label: 'GALLERY' },
    { id: 'contact', label: 'CONTACT' },
  ];

  const toggleMenu = () => setIsOpen((s) => !s);

  const scrollToId = (id?: string) => {
    if (!id) return;
    const el = document.getElementById(id);
    if (!el) return;
    // account for the sticky navbar height so section isn't hidden behind it
    const navbarHeight = 80; // adjust if navbar height changes
    const top = el.getBoundingClientRect().top + window.pageYOffset - navbarHeight;
    window.scrollTo({ top, behavior: 'smooth' });
  };

  return (
    <nav className="bg-black text-white sticky top-0 z-50 shadow-lg">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 md:px-8 py-4">
        {/* Logo */}
        <div className="flex items-center flex-shrink-0">
          <button onClick={() => scrollToId('hero')} aria-label="Go to home">
            <img src="/images/new paradise banquet.png" alt="New Paradise logo" className="h-14 md:h-16 w-auto" />
          </button>
        </div>

        {/* Desktop Navigation Links */}
        <div className="hidden lg:flex items-center gap-8">
          {navLinks.map((link) => (
            <button
              key={link.id}
              onClick={() => scrollToId(link.id)}
              className="text-yellow-500 hover:text-yellow-400 transition-all duration-300 font-semibold text-sm relative group"
            >
              {link.label}
              <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-yellow-400 group-hover:w-full transition-all duration-300"></span>
            </button>
          ))}

          <button
            onClick={() => window.open('https://wa.me/923191113372', '_blank')}
            className="bg-yellow-500 text-black px-6 py-2 rounded-md hover:bg-yellow-400 transition-all duration-300 font-semibold text-sm transform hover:scale-105"
          >
            BOOK NOW
          </button>
        </div>

        {/* Mobile Menu Button */}
        <button
          onClick={toggleMenu}
          className="lg:hidden text-yellow-500 text-3xl font-bold focus:outline-none transition-transform duration-300"
          aria-label="Toggle menu"
          style={{ transform: isOpen ? 'rotate(90deg)' : 'rotate(0deg)' }}
        >
          ☰
        </button>
      </div>

      {/* Mobile Navigation Menu */}
      <div className={`lg:hidden overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`} style={{ visibility: isOpen ? 'visible' : 'hidden' }}>
        <div className="bg-black/95 backdrop-blur-sm border-t border-yellow-500/30 px-4 py-4 space-y-2 animate-slideDown">
          {navLinks.map((link) => (
            <button
              key={link.id}
              onClick={() => {
                scrollToId(link.id);
                setIsOpen(false);
              }}
              className="block text-yellow-500 hover:text-yellow-400 hover:bg-yellow-500/10 px-4 py-3 rounded-md transition-all duration-300 font-semibold text-sm text-left"
            >
              {link.label}
            </button>
          ))}

          <button
            onClick={() => {
              window.open('https://wa.me/923191113372', '_blank');
              setIsOpen(false);
            }}
            className="block bg-yellow-500 text-black px-4 py-3 rounded-md hover:bg-yellow-400 transition-all duration-300 font-semibold text-sm text-center transform hover:scale-105 mt-4"
          >
            BOOK NOW
          </button>
        </div>
      </div>
    </nav>
  );
}
